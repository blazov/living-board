...
#
# Usage: bash artifacts/scripts/cycle-start.sh [branch]
#   branch defaults to "master" if omitted.
#
# What it does:
#   1. Detect whether HEAD is detached (or already on the target branch).
#   2. Switch to the target branch (idempotent).
#   3. Align local branch to origin/branch:
...
if [ "$current_ref" = "DETACHED" ]; then
  echo "[cycle-start] HEAD was detached at $current_sha — switching to $BRANCH"
fi

# Idempotent checkout. If already on the target branch, this is a no-op.
if ! git checkout "$BRANCH"; then
  echo "[cycle-start] ERROR: git checkout $BRANCH failed" >&2
  exit 1
fi

# Fetch the target branch explicitly; do not merge yet. We inspect refs first.
if ! git fetch origin "$BRANCH"; then
  echo "[cycle-start] ERROR: git fetch origin $BRANCH failed" >&2
  exit 1
fi
...
if [ "$local_sha" = "$remote_sha" ]; then
  echo "[cycle-start] already aligned with origin/$BRANCH"
elif git merge-base --is-ancestor "$local_sha" "$remote_sha"; then
  echo "[cycle-start] fast-forwarding $BRANCH from $local_sha to $remote_sha"
  if ! git merge --ff-only "origin/$BRANCH"; then
    echo "[cycle-start] ERROR: fast-forward merge failed" >&2
    exit 1
  fi
else
  # Disjoint or diverged. Local branch is the template seed or similar
  # non-agent state. The agent's canonical state is origin/$BRANCH.
  # Refuse to destroy uncommitted work -- safety gate before reset --hard.
  if ! git diff --quiet || ! git diff --cached --quiet; then
    echo "[cycle-start] ERROR: local $BRANCH ($local_sha) is not an ancestor of origin/$BRANCH ($remote_sha), but the working tree is dirty." >&2
    echo "[cycle-start] HINT: inspect with 'git status' and either commit, stash, or discard before re-running." >&2
    exit 1
  fi
  echo "[cycle-start] local $BRANCH ($local_sha) is not an ancestor of origin/$BRANCH ($remote_sha) — resetting (disjoint-seed path)"
  if ! git reset --hard "origin/$BRANCH"; then
    echo "[cycle-start] ERROR: git reset --hard origin/$BRANCH failed" >&2
    exit 1
  fi
fi
